...
int map_progressive(int variable, int min_fm, int max_fm, int min_to, int max_to){
  float percentage = (variable - min_fm)/(float)(max_fm - min_fm);
  percentage = sqrtf(percentage); 
  int result = percentage*(max_to - min_to) + min_to; 
  return result;
} 
/* USER CODE END 0 */
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if(isLinear){
      ...
      output = map_progressive(HAL_ADC_GetValue(&hadc1),0,4090,0,100);
    }
